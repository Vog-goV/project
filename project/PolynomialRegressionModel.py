...
# === Saving the poly_dataset ===
#name_set = 'models/default.npz' # commented so it accidentaly not to overwrite the set
np.savez_compressed(name_set, X_train=Xp_train, y_train=y_train, X_test=Xp_test, y_test=y_test)
# ------------------------
...
# === Saving the poly_model ===
#name_model = 'models/default.npz' # commented so it accidentaly not to overwrite the model
joblib.dump(ridge_model, name_model)
# ------------------------

print(y_test[1116])
print(ridge_model.predict(Xp_test[1116].reshape(1, -1)))